            ...
            vec![
                named_entity_to_command(
                    &Name {
                        ...
                        description: Some(
                            "Remove the specified output, or all if not given".to_string(),
                        ),
                    },
                    false,
                ),
                named_entity_to_command(
                    &Name {
                        ...
                        long: "add".to_string(),
                        description: Some(
                            "Add an output without changing existing outputs".to_string(),
                        ),
                    },
                    false,
                ),
...
impl<B: Board> AbstractGoState for GoState<B> {
    fn set_searchmoves(&mut self, words: &mut Tokens) -> Res<()> {
        ...
        while let Some(mov) = words
            .peek()
            .and_then(|m| B::Move::from_text(m, &self.pos).ok())
        {
            words.next().unwrap();
...
pub fn select_command<T: NamedEntity + ?Sized>(
    list: &[Box<T>],
    value_autocomplete: bool,
) -> CommandList {
    let mut res: CommandList = vec![];
    for entity in list {
...
pub fn options_options(
    info: Arc<Mutex<EngineInfo>>,
    accept_name_word: bool,
    allow_value: bool,
) -> CommandList {
    let mut res: CommandList = select_command(
