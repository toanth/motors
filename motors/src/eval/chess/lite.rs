...
use crate::eval::chess::{DiagonalOpenness, FileOpenness, flank, pawn_shield_idx};
...
impl<Tuned: LiteValues> GenericLiTEval<Tuned> {
    ...
    fn mobility_and_threats(pos: &Chessboard, us: ChessColor) -> Tuned::Score {
        ...
        let their_king = pos.king_square(!us);
        let king_zone = Chessboard::normal_king_attacks_from(their_king);
        ...
        for piece in ChessPieceType::non_pawn_pieces() {
            for square in pos.col_piece_bb(us, piece).ones() {
                let attacks = pos.threatening_attacks(square, piece, us, &generator);
                all_attacks |= attacks;
        ...
        // Idea from sirius
        let num_attacked = (flank(their_king, !us) & pos.player_bb(!us) & all_attacks).num_ones();
        score += Tuned::flank_attack() * num_attacked;
        let num_defended = (flank(pos.king_square(us), us) & pos.player_bb(us) & all_attacks).num_ones();
        score += Tuned::flank_defense() * num_defended;
        score
    }
