    ...
    fn negamax(
        ...
        while let Some((mov, move_score)) = move_picker.next(&move_scorer, &self) {
            ...
            if self.search_stack[ply].tried_moves.len() == 1 {
                ...
                // extend pawn moves to the second-to-last rank to avoid cutting off before a potential promotion
                if mov.piece_type() == Pawn && mov.dest_square().is_pawn_start_rank() {
                    reduction -= 1;
                }
