...
mod tests {
    ...
    fn test_pos(pos: FairyBoard, fen: &str, expected: &[u64], start_time: Instant) {
        for (i, &expected) in expected.iter().enumerate() {
            if cfg!(debug_assertions) && expected > 1_000_000 {
                break;
            }
            let i = i + 1;
    ...
    fn perft_tests() {
        ...
        for (testcase, res) in fens {
            ...
            test_pos(pos, testcase, res.as_slice(), start_time);
        }
    }
    ...
    fn shakmaty_tests() {
        ...
        for (fen, expected) in fens {
            ...
            test_pos(pos, fen, expected.as_slice(), start_time);
        }
    }
}
