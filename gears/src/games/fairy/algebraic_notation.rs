...
fn format_origin(f: &mut Formatter, mov: Move, pos: &Board, moves: &[Move], piece: PieceId) -> fmt::Result {
...
impl<'a> MoveParser<'a> {
    ...
    fn parse_stm(&mut self, pos: &Board) -> Res<()> {
    ...
    fn parse_castling(&mut self, board: &Board) -> Option<Move> {
        ...
        Some(Move::new(from, to, kind, false))
    }

    fn parse_piece_char(c: char, pos: &Board) -> Option<PieceId> {
    ...
    fn parse_piece(&mut self, pos: &Board) -> Res<()> {
    ...
    fn parse_square_rank_or_file(&mut self, pos: &Board) -> Res<()> {
    ...
    fn parse_second_square(&mut self, pos: &Board) {
        ...
        self.advance_char();
        if let Ok((rank, rank_str)) = Self::parse_rank(self.remaining())
            && let Ok(square) = Square::algebraic(file, rank as usize + 1)
        {
            self.num_bytes_read += rank_str.len();
    ...
    fn parse_usi_square(&mut self, pos: &Board) -> Option<Square> {
    ...
    fn parse_promotion(&mut self, pos: &Board) -> Res<()> {
    ...
    fn check_rank(&self, rank: DimT, pos: &Board, descr: &str) -> Res<()> {
        if rank >= pos.height() {
            bail!(
    ...
    fn check_file(&self, file: DimT, pos: &Board, descr: &str) -> Res<()> {
        let width = pos.width();
        if file >= width {
            bail!(
    ...
    fn matching_castling_move(&self, mov: Move, pos: &Board) -> bool {
        ...
        let start = Square::from_rank_file(start_rank, start_file);
        let target = Square::from_rank_file(target_rank, target_file);
        if start != from {
            return false;
        }
    ...
    fn is_matching_pseudolegal(&self, mov: Move, pos: &Board, cmp_promo: bool) -> bool {
    ...
    fn error_msg(&self, board: &Board, original_piece: PieceId) -> Res<Move> {
        let us = board.active;
            ...
            if let Some(file) = file {
                match rank {
                    Some(rank) => {
                        ...
                        (square.to_string(), Bitboard::single_piece_for(square, board.size()))
                    }
                    None => (format!("the {} file", file_to_char(file)), Bitboard::file_for(file, board.size())),
                }
            } else if let Some(rank) = rank {
                (format!("rank {rank}"), Bitboard::rank_for(rank, board.size()))
            } else {
        ...
        if let Some(res) = board.calc_player_result(&NoHistory::default()) {
            ...
            if piece.is_empty() {
                ...
                let c =
                    piece.color().map(|c: Color| c.name(board.settings()).bold().to_string()).unwrap_or(String::new());
                additional = format!(
                    " (there is a {c} {0} on {1})",
        ...
        if original_piece == PieceId::empty() && from_bb.is_single_square() {
    ...
    fn check_check_checkmate_captures(&self, mov: Move, board: &Board) -> Res<()> {
