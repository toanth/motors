...
use crate::games::fairy::{Board, Color, FairyCastleInfo, Square};
use crate::games::{BoardHistDyn, ColorTrait, NoHistory, ZobristHistory, chess};
...
#[test]
fn startpos_test() {
    let pos = Board::variant_simple("atomic").unwrap();
...
#[test]
fn capture_test() {
    let pos = Board::from_fen_for("atomic", "r1bqkbnr/pp1p3p/1Qn1ppp1/8/8/4P3/PPPP1PPP/RNB1KB1R b KQkq - 0 1", Strict)
        .unwrap();
    assert_eq!(pos.active_player(), Color::second());
    let mov = Move::from_text("a7b6", &pos).unwrap();
...
fn explosion_win_test() {
    let pos =
        Board::from_fen_for("atomic", "rnbqkbnr/pp2pppp/3p4/2p1N3/8/8/PPPPPPPP/RNBQKB1R w KQkq - 0 2", Strict).unwrap();
...
#[test]
fn ep_test() {
    let pos = Board::from_fen_for("atomic", "8/2bpk3/8/4P3/8/8/3K3B/8 b - - 0 1", Strict).unwrap();
    let before_ep = pos.make_move_from_str("d7d5").unwrap();
    assert_eq!(before_ep.fen_no_rules(), "8/2b1k3/8/3pP3/8/8/3K3B/8 w - d6 0 2");
    assert_eq!(before_ep.ep, Some(Square::algebraic('d', 6).unwrap()));
    assert_eq!(before_ep, Board::from_fen_for("atomic", &before_ep.fen_no_rules(), Relaxed).unwrap());
    let ep = Move::from_text("e5d6", &before_ep).unwrap();
    let after_ep = before_ep.make_move(ep).unwrap();
    assert!(after_ep.royal_bb_for(Color::second()).is_zero());
...
#[test]
fn no_suicide_test() {
    let pos = Board::from_fen_for("atomic", "2K2k2/3p4/8/8/8/7B/8/8 w - - 0 1", Strict).unwrap();
    assert!(pos.debug_verify_invariants(Strict).is_ok());
    assert!(pos.clone().make_move_from_str("h3d7").is_err());
    assert!(pos.make_move_from_str("c8d7").is_err());
    let pos = Board::from_fen_for("atomic", "8/8/8/5Q2/8/2K5/2k5/8 b - - 0 1", Strict).unwrap();
    assert!(pos.debug_verify_invariants(Strict).is_ok());
    assert!(pos.make_move_from_str("f5c2").is_err());
    let pos = Board::from_fen_for("atomic", "5R2/8/8/8/4k3/5pK1/8/8 w - - 0 1", Strict).unwrap();
    ...
    let pos = Board::from_fen_for("atomic", "2Kk4/3p4/8/8/8/7B/8/8 w - - 0 1", Strict).unwrap();
...
#[test]
fn check_test() {
    let pos = Board::from_fen_for("atomic", "rnbqk1r1/1p2p2p/p1pp1pp1/q1N5/3PP3/8/PPP2PPP/R2QKB1R w KQq - 0 1", Strict)
        .unwrap();
    ...
    let pos = Board::from_fen_for("atomic", "8/8/8/3k3b/8/8/4N1P1/3K4 w - - 0 1", Strict).unwrap();
    assert!(!pos.is_in_check());
    ...
    let pos = Board::from_fen_for("atomic", "8/8/8/7b/8/7N/6P1/2kK4 w - - 0 1", Strict).unwrap();
    assert!(!pos.is_in_check());
    let pos = pos.make_nullmove().unwrap();
...
fn atomic_check_test() {
    let pos = Board::from_fen_for("atomic", "8/8/8/7b/5nn1/7N/6P1/1k1K4 w - - 0 1", Strict).unwrap();
    let new_pos = pos.make_move_from_str("h3f4");
    assert!(new_pos.is_err());

    let pos =
        Board::from_fen_for("atomic", "r1b1k1nr/pp1pQ1pp/n1p2p2/1B2p3/1P5q/4P3/P1PP1PPP/RN2K2R b KQkq - 0 1", Strict)
            .unwrap();
    assert!(pos.is_in_check());
...
fn checkmate_stalemate_test() {
    let pos =
        Board::from_fen_for("atomic", "rn1qkb1r/pppBpppp/5n2/3p4/6b1/4PQ2/PPPP1PPP/RNB1K1NR b KQkq - 0 1", Strict)
            .unwrap();
    assert!(pos.is_in_check());
...
#[test]
fn castling_test() {
    let pos = Board::from_fen_for("atomic", "6q1/8/8/8/8/3k4/6P1/4K2R b K - 0 1", Strict).unwrap();
    assert!(pos.castling_info.can_castle(Color::first(), Kingside));
    let new_pos = pos.make_move_from_str("g8g2").unwrap();
    assert!(!new_pos.castling_info.can_castle(Color::first(), Kingside));
    assert_eq!(new_pos.fen_no_rules(), "8/8/8/8/8/3k4/8/4K3 w - - 0 2");
    assert!(new_pos.is_draw_slow(&NoHistory::default()));

    let pos = Board::from_fen_for("atomic", "8/8/8/1q6/8/8/4k3/R3K3 w Q - 0 1", Strict).unwrap();
    let castling_move = Move::from_text("e1a1", &pos).unwrap();
    assert!(pos.is_move_legal(castling_move));
    let pos = Board::from_fen_for("atomic", "8/8/8/8/8/8/4k3/R3K2q w Q - 0 1", Strict).unwrap();
    let castling_move = Move::from_text("e1a1", &pos).unwrap();
    assert!(pos.is_move_legal(castling_move));
    let new_pos = pos.make_move(castling_move).unwrap();
    assert!(new_pos.in_check_bb(Color::first()).is_zero());
    assert_eq!(new_pos.castling_info, FairyCastleInfo::default());

    let pos = Board::from_fen_for("atomic", "8/8/8/8/8/8/5k2/R3K2q w Q - 0 1", Strict).unwrap();
    let new_pos = pos.make_move_from_str("e1a1");
    assert!(new_pos.is_err());

    let pos = Board::from_fen_for("atomic", "8/8/8/8/8/8/3k4/R3K2q w Q - 0 1", Strict).unwrap();
...
fn draw_test() {
    ...
    let mut pos = Board::from_fen_for("atomic", "8/8/8/4K2Q/8/8/8/1k6 w - - 0 1", Strict).unwrap();
