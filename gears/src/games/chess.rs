...
use crate::general::board::{
    ...
    NameToPos, Strictness, UnverifiedBoard,
};
...
mod tests {
    ...
    fn simple_perft_test() {
        ...
        assert_eq!(perft(Depth::new_unchecked(3), board, false).nodes, 16790);
        ...
        .unwrap();
        let perft_res = perft(Depth::new_unchecked(4), board, true);
        ...
        .unwrap();
        assert_eq!(
            perft(Depth::new_unchecked(4), board, false).nodes,
            1_187_103
        );
    }
    ...
    fn weird_position_test() {
        ...
        let board = Chessboard::from_fen(fen, Strict).unwrap();
        assert_eq!(board.active_player, White);
        assert_eq!(perft(Depth::new_unchecked(3), board, false).nodes, 568_299);
        assert_eq!(perft(Depth::new_unchecked(3), board, true).nodes, 568_299);
