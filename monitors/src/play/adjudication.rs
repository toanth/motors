...
use gears::{AdjudicationReason, GameOver, GameOverReason, GameResult, GameState, MatchResult, player_res_to_match_res, PlayerResult};
...
impl Adjudicator {
    ...
    pub fn new(resign: Option<ScoreAdjudication>, draw: Option<ScoreAdjudication>, max_moves_until_draw: usize) -> Self {
    ...
    fn adjudicate_resignation<B: Board>(&mut self, state: &ClientState<B>) -> Option<MatchResult> {
        ...
        if state.ply_count() < resign.start_after {
            ...
            return Some(MatchResult{ result: GameResult::Draw, reason: GameOverReason::Adjudication(AdjudicationReason::Adjudicator(message)) });
        }
        let white_score = state.get_engine(White).current_match.as_ref().unwrap().search_info.as_ref()?.score;
        let black_score = state.get_engine(Black).current_match.as_ref().unwrap().search_info.as_ref()?.score;
        let mut player = None;
        ...
        if resign.counter >= resign.move_number {
            ...
            let game_over = GameOver { result: PlayerResult::Lose, reason: GameOverReason::Adjudication(AdjudicationReason::Adjudicator(message)) };
    ...
    fn adjudicate_draw<B: Board>(&mut self, state: &ClientState<B>) -> Option<MatchResult> {
        let draw = self.draw?;
        if state.ply_count() < draw.start_after { // < instead of <= because start_after == 0 means to start immediately
            return None
        }
        let mut counter = draw.counter;
        if state.get_engine(White).current_match.as_ref().unwrap().search_info.as_ref()?.score.abs() < draw.score_threshold
            && state.get_engine(Black).current_match.as_ref().unwrap().search_info.as_ref()?.score.abs() < draw.score_threshold {
            counter += 1;
        }
        if counter >= draw.move_number {
            let message = format!("Both engine's score was less than {0} cp for {counter} plies in a row", draw.score_threshold.0);
            let game_over = GameOver { result: PlayerResult::Draw, reason: GameOverReason::Adjudication(AdjudicationReason::Adjudicator(message)) };
