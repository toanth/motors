...
use gears::general::common::{NamedEntity, parse_int_from_str, Res, select_name_static, StaticallyNamedEntity};
...
fn select<F>(names: Vec<&'static str>, func: F) -> TextSelection<F> {
    TextSelection { names, func, description: None }
}
...
type Command<B> = TextSelection<for<'a> fn(MutexGuard<Client<B>>, &'a mut SplitWhitespace) -> Res<()>>;
...
impl<B: Board> TextInputThread<B> {
    pub fn input_loop(ugi_client: Weak<Mutex<Client<B>>>) {
        ...
        loop {
            ...
            if let Err(err) = stdin().read_line(&mut input) {
                if let Some(ugi_client) = ugi_client.upgrade() { // Else, the match has ended, so don't bother printing an error message.
                    ugi_client.lock().unwrap().show_error(
                        format!("Couldn't get input: {}", err.to_string()).as_str(),
                    )
                };
                break;
            }
            ...
            match input_thread.handle_input(client, input.split_whitespace(), input) {
                Ok(continue_running) => if !continue_running { break; },
                Err(e) => {
                    ugi_client.upgrade().inspect(|client| client.lock().unwrap().show_message(Warning, &format!("Ignoring input. {e}")));
                }
            }
        }
    }

    fn handle_input(&self, mut ugi_client: Arc<Mutex<Client<B>>>, mut words: SplitWhitespace, input: &str) -> Res<bool> {
        ...
        if command.eq_ignore_ascii_case("help") {
            ...
            match B::Move::from_text(input, &client.board()) {
                Ok(mov) => {
                    let active_player = client.active_player().ok_or_else(|| "Ignoring move because the game is over".to_string())?;
                    client.play_move(mov).map_err(|err| format!("Ignoring input: {err}"))?;
                    // `play_move` will have stopped the clock by now.
                    assert!(client.state.get_player(active_player).thinking_since().is_none());
                    return Ok(true);
                }
                Err(err) => {
                    let func = select_name_static(command, &self.commands, "command", B::game_name()).map_err(|msg| format!("'{command}' is not a pseudolegal move: {err}.\nIt's also not a command: {msg}\nType 'help' for more information."))?.func;
                    func(client, &mut words)?;
                }
            }
        }
        if let Some(w) = words.next() {
            ugi_client.lock().unwrap().show_message(Warning, &format!("Ignoring extra input starting with '{w}' after the '{command}' command"));
        }
    ...
    fn print_help(commands: &[Command<B>], words: &mut SplitWhitespace) -> Res<()> {
        if let Some(name) = words.next() {
            let desc = select_name_static(name, commands, "command", B::game_name())?.description.unwrap_or("No description available");
            println!("{}", desc);
        } else {
            println!("Input either a move (most formats based on algebraic notation are recognized) or a command. Valid commands are:");
            for cmd in commands.into_iter() {
                println!("{:15}  {description}", cmd.names.iter().map(|c| format!("'{c}'")).intersperse(", ".to_string()).collect::<String>(),
    ...
    fn get_side(client: &MutexGuard<Client<B>>, words: &mut SplitWhitespace, default_player: DefaultPlayer) -> Res<Color> {
        match words.next().unwrap_or_default().to_ascii_lowercase().as_str() {
            ...
            x => {
                ...
                if player == Active {
                    Ok(client.active_player().ok_or_else(|| "No color given and there is no active player (the match isn't running)".to_string())?)
    ...
    fn list_moves(mut client: MutexGuard<Client<B>>) {
        ...
        println!("{}", board.legal_moves_slow().map(|m| m.to_extended_text(&board)).intersperse(", ".to_string()).collect::<String>());
    }
    ...
    fn random_move(mut client: MutexGuard<Client<B>>) -> Res<()> {
        ...
        let mov = board.random_legal_move(&mut rng).ok_or_else(|| format!("There are no legal moves in the current position ({}){reason}", board.as_fen(),
    ...
    fn handle_stop(mut client: MutexGuard<Client<B>>, words: &mut SplitWhitespace) -> Res<()> {
        ...
        if !client.state.get_player(side).is_engine() {
            return Err(format!("The {side} player is a human and not an engine, so they can't be stopped"));
        }
    ...
    fn handle_position(mut client: MutexGuard<Client<B>>, words: &mut SplitWhitespace) -> Res<()> {
        ...
        for mov in words {
            let mov = B::Move::from_compact_text(mov, &client.board()).map_err(|err| format!("Couldn't parse move: {err}"))?;
    ...
    fn handle_set_player(mut client: MutexGuard<Client<B>>, words: &mut SplitWhitespace) -> Res<()> {
        let side = Self::get_side(&client, words, NoPlayer)?;
        let name = words.next().ok_or_else(|| format!("Missing the name of the new player (e.g. 'human'). Loaded players are {}", client.state.players.iter().map(|p| p.get_name()).intersperse(", ").collect::<String>()))?;
        ...
        for (idx, player) in client.state.players.iter().enumerate() {
            if player.get_name().to_lowercase() == name.to_lowercase() {
                if idx != white && idx != black { // TODO: Remove this restriction, simply build a new one
        ...
        if found {
            Err(format!("The player '{name}' is already playing in this match"))
        } else {
    ...
    fn handle_load_player(ugi_client: Arc<Mutex<Client<B>>>, words: &mut SplitWhitespace) -> Res<()> {
    ...
    fn handle_ui(mut client: MutexGuard<Client<B>>, words: &mut SplitWhitespace) -> Res<()> {
        match words.next() {
            None => {
                let infos = client.outputs.iter().map(|o| format!("{name}: {}", o.description().unwrap_or("<no description>"), name = o.short_name())).join("\n");
                client.show_message(Info, &infos);
            }
            Some(mut name) => {
                let mut replace = true;
                if name == "add" {
                    name = words.next().ok_or_else(|| "Expected an output name after 'add'".to_string())?;
                    replace = false;
                } else if name == "remove" {
                    name = words.next().ok_or_else(|| "Expected an output name after 'remove'".to_string())?;
                    match client.outputs.iter().find_position(|o| o.matches(name)) {
                        None => return Err(format!("There is no output with name '{name}' currently in use")),
                        Some((idx, _output)) => {
    ...
    fn handle_print(mut client: MutexGuard<Client<B>>, words: &mut SplitWhitespace) -> Res<()> {
        match words.next().unwrap_or_default() {
            "" => client.show(),
            x => {
                output_builder_from_str(x, &client.all_outputs)?.for_client(&client.state)?.show(&client.state);
            }
        }
        Ok(())
    }
    ...
    fn handle_info(mut client: MutexGuard<Client<B>>, words: &mut SplitWhitespace) -> Res<()> {
        ...
        match client.state.get_player_mut(color) {
            Player::Engine(engine) => {
                println!("Name: {name} ({ugi_name}), author: {author}", name = engine.display_name, ugi_name = engine.ugi_name, author = engine.author);
        ...
        if client.match_state().status == Ongoing {
            println!("Remaining time: {}", client.state.get_player(color).get_time().unwrap().remaining_to_string(client.state.thinking_since(color)))
        }
        Ok(())
    }
    ...
    fn handle_send_ugi(mut client: MutexGuard<Client<B>>, words: &mut SplitWhitespace) -> Res<()> {
        ...
        if !client.state.get_player_mut(player).is_engine() {
            return Err(format!("The {player} player is not an engine and can't receive UGI commands"));
        }
...
impl<B: Board> Input<B> for TextInput {
    fn assume_control(&mut self, ugi_client: Arc<Mutex<Client<B>>>) {
        self.handle = Some(Builder::new().name("Text input thread".to_string()).spawn(move || TextInputThread::input_loop(Arc::downgrade(&ugi_client))).unwrap());
    }
